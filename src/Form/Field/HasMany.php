...
class HasMany extends Field
{
    ...
    public function getValidator(array $input)
    {
        ...
        foreach ($rules as $column => $rule) {
            foreach (array_keys($input[$this->column]) as $key) {
                ...
                $_rule =  $this->fixRules($rule, $key);
                $newRules["{$this->column}.$key.$column"] = $_rule;
    ...
    /**
     * 修正 HasMany Unique 驗證
     *
     * @param [type] $rules
     * @param [type] $key
     * @return void
     */
    private function fixRules($rules, $key) {
        $_rules = $rules;
        foreach($rules as $idx => $rule) {
            if (stripos($rule, "{{__id}}") !== false) {
                $_rules[$idx] = str_replace('{{__id}}', $key, $rule);
            } else {
                $_rules[$idx] = $rule;
            }
        }
