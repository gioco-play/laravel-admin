...
trait ModelTree
{
    ...
    public function allNodes()
    {
        ...
        return $self->orderBy('sort_order')->get()->toArray();
    }
    ...
    protected function buildSelectOptions(array $nodes = [], $parentId = 0, $prefix = '', $space = '&nbsp;')
    {
        ...
        if (empty($nodes)) {
            $nodes = [];
            $_nodes = $this->allNodes();
            if (count($_nodes)===1) {
                $parentId = current($_nodes)['parent_id'];
            }
            static::flatChildren($nodes, $_nodes);
        }
     ...
     * @return void
     */
    public static function flatChildren(&$nodes, $children = null) {
        foreach ($children as $node) {
            if (isset($node['children'])) {
                static::flatChildren($nodes, $node['children']);
            }
            $nodes[] = $node;
        }
    }
